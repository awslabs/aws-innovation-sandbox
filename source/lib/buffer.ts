...
export class InnovationSandbox extends cdk.Stack {
  ...
  constructor(
    ...
    l0_role.addToPolicy(  new iam.PolicyStatement({
    ...
    l6_role.addToPolicy(  new iam.PolicyStatement({
      effect: iam.Effect.ALLOW,
      resources: [cdk.Fn.sub("arn:aws:logs:${AWS::Region}:${AWS::AccountId}:log-group:/aws/lambda/${AWS::StackName}*:*")],
      actions: ["logs:CreateLogGroup",
                "logs:CreateLogStream",
    ...
    const l6policystatement =  new iam.PolicyStatement({
      effect: iam.Effect.ALLOW,
